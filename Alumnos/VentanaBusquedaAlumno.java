...
public class VentanaBusquedaAlumno extends JDialog implements ItemListener{
	...
	private JTextField txtLunes;
	private JTextField txtMartes;
	private JTextField txtMiercoles;
	private JTextField txtJueves;
	private JTextField txtViernes;
	private JTextField txtSabado;
	...
	public VentanaBusquedaAlumno() {
		...
		btnVolver.setBounds(330, 440, 110, 23);
		...
		btnLimpiar.setBounds(25, 440, 110, 23);
		...
		btnGuardar.setBounds(330, 400, 110, 23);
		...
		btnAsistencia.setBounds(175, 400, 110, 23);
		...
		btnImprimir.setBounds(25, 400, 110, 23);
	...
	private void mostrarBusqueda(String campo, String valor) {
		...
		if(valor.contentEquals("")) {
			...
			limpiarCampos();
			String resAlumno[] = ABMCAlumnos.buscarAlumno(campo, valor);
			...
			int idGrupoFamiliar = Integer.parseInt(resAlumno[10]);
			chckbxActivo.setEnabled(idGrupoFamiliar > 0? true : false);
			
			if(resAlumno[0] != null) {
				
				idAlumno = resAlumno[0];
				txtLegajo.setText(idAlumno);
				txtNombre.setText(resAlumno[1]);
				txtApellido.setText(resAlumno[2]);
				txtDni.setText(resAlumno[3]);
				String[] parts = resAlumno[4].split("-");
				txtAño.setText(parts[0]);
				txtMes.setText(parts[1]);
				txtDia.setText(parts[2]);
				txtDireccion.setText(resAlumno[5]);
				txtTelefono.setText(resAlumno[6]);
				txtEmail.setText(resAlumno[7]);
				txtFechaIngreso.setText(resAlumno[8]);
				
				if(resAlumno[9].equals("1"))
					chckbxActivo.setSelected(true);
				else
					chckbxActivo.setSelected(false);
				
				estadoAnterior = resAlumno[9];
				int i=0;
				for(i=0 ; i < respuestaCurso.length ; i++) {
					
					if(respuestaCurso[i][0].equals(resAlumno[11]))
						break;
				}
				comboBoxCurso.setSelectedIndex(i);								
				
				String resDiasCursado[][] = ABMCDiasCurso.buscarDiasCursado(resAlumno[11]);
				int u=0;
	...
	private void gurdarCambios() {
		...
		String registroModificado[] = { idAlumno,
										txtNombre.getText(),
										txtApellido.getText(),
										txtDni.getText(),
		...
		if(checkCampos()) {
			...
			if(!estado.contentEquals(estadoAnterior)) {
				...
				String respuesta[] = ABMCAlumnos.buscarAlumno("ID", idAlumno);
	...
	private void limpiarCampos() {
		...
		txtLunes.setText(null);
		txtMartes.setText(null);
		txtMiercoles.setText(null);
		txtJueves.setText(null);
		txtViernes.setText(null);
		txtSabado.setText(null);
